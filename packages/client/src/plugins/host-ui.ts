...
export interface PluginHostUI {
    ...
    readonly navigationTrustedRuntimes?: ReadonlySet<KelpiRuntime> | undefined;
...
/** Runtime identity comes from the host, never from plugin-supplied arguments. */
export function canAccessNavigation(host: PluginHostUI | null, runtime: KelpiRuntime): boolean {
    return host !== null && (host.runtime === runtime || host.navigationTrustedRuntimes?.has(runtime) === true);
}
...
export function requestHostUI(host: PluginHostUI | null, runtime: KelpiRuntime, method: string, args: JsonObject): JsonValue | Promise<JsonValue> {
    if ((NAVIGATION_UI_METHODS as readonly string[]).includes(method)) {
        if (!canAccessNavigation(host, runtime)) throw new Error('Navigation is unavailable for this daemon in this window.');
