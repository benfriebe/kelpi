 ...
 * wrote a bare `nex` into every hook. This CLI ships several ways — a workspace checkout run as
 * `node dist/nex.js`, a `pnpm link`, a launcher inside `Nex.app/Contents/Resources/cli` — so the
 * command has to be *resolved* rather than assumed:
 *
 *   1. `--command <prefix>` wins outright (scripts, tests, and anyone with an opinion);
 *   2. otherwise, if a `nex` on `PATH` resolves to THIS binary, the hooks get the bare `nex` —
 ...
 * does not read `~/.zshrc`. It inherits the PATH of the agent process. So a bare `nex` only
...
export interface SelfResolution {
    ...
    /** True when the prefix is the bare `nex` because PATH really resolves to us. */
 ...
 *
 * `argv[1]` is the bundle (`dist/nex.js`) under both `node dist/nex.js` and a shebang exec, and
 * it is realpath-resolved so a `/usr/local/bin/nex` symlink reports the file it points at.
 */
...
/**
 * Does a `nex` on `PATH` resolve to this binary?
 *
 * Two shapes count, because the packaged app installs a *launcher* rather than the bundle: the
 * PATH entry can realpath to the running file itself, or to a sibling named `nex` in the same
...
export function findSelfOnPath(
    ...
    const siblingLauncher = path.join(path.dirname(executable), 'nex');
    const entries = (pathValue ?? '').split(':').filter((entry) => entry.length > 0);
    for (const entry of entries) {
        const candidate = path.join(entry, 'nex');
...
export function resolveHookCommand(options: ResolveCommandOptions, fsys: InstallFs): SelfResolution {
    ...
    if (override !== undefined && override.length > 0) {
        return { executable, command: override, onPath: override === 'nex' };
    }
    const entry = findSelfOnPath(executable, options.pathValue, fsys);
    if (entry !== null) return { executable, command: 'nex', onPath: true, pathEntry: entry };
    ...
    return { executable, command: executable === null ? 'nex' : shellQuote(executable), onPath: false };
}
