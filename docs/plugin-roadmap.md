...
| Full Settings presentation | **Phases 1 and 2 implemented on `feature/plugin-settings-contracts` and `feature/plugin-settings-presenter`.** A shared settings model (closure-free section and field descriptors, host-owned validation, drafts and routing) renders General, Workspaces and Appearance's plain rows, and a plugin view declaring `settings.window` can be selected as the Settings presenter with the bundled panel as the recovery floor. Plugins, Remote, Profiles, Keybindings, Labels, Repositories, Web, both key recorders, Appearance's theme importer and every destructive confirmation stay native. | Phase 3: a Settings Lab example, `scripts/scenarios/plugin-settings-presenter.mjs`, audit steps and a validation record entry. |
