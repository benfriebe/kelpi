...
export const PLUGIN_PLACEMENTS = ['pane', 'sidebar.primary', 'sidebar.secondary', 'panel.bottom', 'topbar', 'statusbar', 'workspace', 'settings'] as const;
...
export function decodePluginManifest(raw: unknown): PluginManifest {
    ...
    const containers = array(contributes['containers']).map((raw): PluginContainerDefinition => {
        ...
        if (places.includes('pane')) throw new Error('containers require workbench placements; pane views own their individual saved state');
