    ...
    for (const component of list(components)) {
        ...
        for (const [index, attempt] of attempts.entries()) {
            ...
            if (e) {
                ...
                for (const signoff of list(manifest?.auditVisualSignoffs)) {
                    const step = list(raw?.steps).find(s => s?.id === signoff?.id);
                    if (!step || signoff.runId !== run.runId || signoff.head !== run.start.head) continue;
                    if (signoff.verdict === 'failed') { attemptVerdicts.push('failed'); reasons.push(`visual review failed: ${step.id}`); continue; }
            ...
            assessment.attempts.push({ verdict, counts: checked?.counts ?? null, outstandingVisuals: checked?.visuals ?? [], missingCategories: checked?.missing.length ?? (verdict === 'unverified' ? 1 : 0) });
        }
