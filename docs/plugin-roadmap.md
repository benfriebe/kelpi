...
| Full Settings presentation | **In progress on `feature/plugin-settings-contracts`.** A shared settings model (closure-free section and field descriptors, host-owned validation, drafts and routing) renders the General and Workspaces tabs; every other section is native and the dialog is unchanged. No plugin API yet. | Phase 2: a selectable `settings.window` presenter with bundled recovery, keeping Plugins, Remote, Profiles, the key recorders and destructive confirmations native; phase 3: a Settings Lab and live acceptance. |
