...
export function BatchPanel(props: BatchPanelProps): ReactElement {
    ...
    return (
        <div
            ...
            /*
             * S35: each BAND pads itself, and the two `Divider()`s run edge to edge between them
             * — `WebBatchInspectPanel.swift:53-64` is a `VStack(spacing: 0)` with no padding of
             * its own, and the bands inside it are deliberately unequal (header 10/5, list 8/6,
             * empty hint 10/8, footer 10/6). A uniform `p-2` + `gap-1.5` on the container
             * flattened that step to 8/6 everywhere, so the title and the Cancel/Send row sat as
             * close to the pane edge as the rows they frame — and it inset the rules by 8 px,
             * where the shipped dividers span the panel.
             */
        ...
        >
             ...
             */}
            <div className="flex items-center gap-1.5 px-2.5 py-[5px] text-[11px]">
            ...
            {session.items.length === 0 ? (
                ...
                <div
                    ...
                    className="flex flex-col gap-1 overflow-y-auto px-2 py-1.5"
                        ...
                        return (
                            ...
                            >
                                ...
                                <button
                                    ...
                                    /*
                                     * S29: `WebBatchInspectPanel.swift:198-204` frames this at
                                     * 18 × 18 with a `.contentShape(Rectangle())` — the same box
                                     * as the numbered chip at the row's other end. The port drew
                                     * the glyph in a line box (16.39 × 15.4), so one end of the
                                     * row was a control and the other was a character.
                                     */
                                    className="flex h-[18px] w-[18px] shrink-0 items-center justify-center text-[11px]"
                                    style={{ color: tokens.textTertiary }}
                                    onClick={(event) => {
