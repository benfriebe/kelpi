...
Tested revision: **`3467866`** (the presenter host `bd9cd3e`, Pane Lab and the scenario `8549556`, and the review follow-up `3467866`). Full verification battery: `node scripts/verify.mjs --full` **passed unretried at `3467866` in 25.2 min** (typecheck, root tests 8,217 passed with 1 skipped, shell tests, build bundles, all scenarios hidden 5.8 min, full audit 18.4 min, packaged smoke 69/69); the only leak warnings are the three inert workbench-slot ones. The first cut (`8549556`) also passed, in a lane that needed the battery's isolated retry: `plugin-pane-chrome`, `plugin-browser-features`, `terminal-scrollback-resize` and `dock-bounce-stop-only` went red together once and green alone, which is a loaded or occluded machine rather than this branch; the scenario's `stty size` reads were hardened afterwards regardless. The worktree was bootstrapped from the verified vendor
...
| Check | Result |
| --- | --- |
| `pnpm check` | All typechecks pass (protocol, core, daemon, cli, plugin-sdk, client, shell). Root vitest **8,217 passed, 1 skipped** (the existing optional database skip); shell **870 passed**. The new tests add exactly **45** (**8,172** before them, which is phase A's own number): `pane-chrome/presenter` (14: the band rectangle's three-sided ring inset and its short-band floor, the clip path's union and its empty case, the window's failure and painted latches, `retainPaneChromeHeights`, and the ref minter's per-key stability, list separation, pane scoping and opacity), `features/pane-lab` (27, the shipped example driven through the real presenter host over the real projection and the real per-pane surface), and `plugin-sdk/tests/pane-chrome` (4, the packaged `browser.js` marshalling every call, keeping `ui.focusPane`'s two-argument form, and staying independent of the settings feed). |
...
| Live acceptance, hidden | `node scripts/scenario.mjs plugin-pane-chrome --window hidden --no-build`: **35/35**, 0 failed, in 22.1 s. The scenario declares `windowPlacement = 'offscreen'`, so a hidden run gives it its own instance at that placement: check 5 drives a real native `WebContentsView`, and `ui-audit/lib/placement.mjs` records that a zero-opacity frame with anything in front of it has its native views' input dropped. |
| The pair, twice | `node scripts/scenario.mjs plugin-pane-chrome plugin-remote --window hidden --no-build` run twice: **35/35 + 13/13** and **35/35 + 13/13**, 0 failed, with the runner's leak post-condition clean both times. |
...
| Whole live scenario (attach, 35 checks, two failure-and-retry cycles, a daemon restart, a phone round trip) | **22.1 s hidden / 22.5 s onscreen** | not measured; by construction one attach, one document and one feed per pane |
...
| **DRAG** The pane-move gesture was lost | Reported as an unavoidable cost of the geometry; it is not. `beginPaneDrag(paneID)` says a press in a band started one, and the host runs its own gesture from the next pointer move. | `features/pane-lab.test.ts` "starts the host's pane-move gesture from a press on the band's title" and "refuses a drag for a pane the frame does not carry"; scenario: "a drag started from the presenter's band moves the pane through the host's own gesture", measured as the pane's rect changing. |
...
| **Doc nit** | The roadmap said twelve presenter calls and eleven pane chrome methods; `ui.reportPresenterReady` pre-existed, so ten were new (eleven with `beginPaneDrag`). | The roadmap now says so. |
...
- `packages/client/src/pane-chrome/`: `presenter.ts` (the frame DTO, the twelve `ui.*` methods, the
  ...
  store, and the presenter budgets on the contract.
