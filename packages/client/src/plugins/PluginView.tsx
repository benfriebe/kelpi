...
import { PluginHostUIContext, requestHostUI, WORKBENCH_UI_METHODS } from './host-ui';
...
export function PluginView(props: PluginViewProps): ReactElement {
    ...
    useEffect(() => {
        ...
        const handleReady = (event: MessageEvent): void => {
            ...
            channel.port1.onmessage = ({ data }) => {
                ...
                void (async () => {
                    ...
                    if ((WORKBENCH_UI_METHODS as readonly unknown[]).includes(data['method'])) return requestHostUI(latestHostUI.current, runtime, String(data['method']), args);
        ...
        return () => { disposed = true; clearTimeout(readinessTimer); ownerWindow.removeEventListener('message', handleReady); observer.disconnect(); offEvents(); port.current?.close(); port.current = null; if (lease) void pluginRequest(runtime, 'release', { lease }).catch(() => {}); };
    }, [runtime, pluginID, viewID, paneID, workspaceID, plugin?.revision, plugin?.instanceID, unavailable, connection, attempt]);
    useEffect(() => {
