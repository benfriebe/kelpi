...
Last reviewed: **2026-09-24**, against merged main `058c8f4` ([#263](https://github.com/benfriebe/kelpi/pull/263), over #261's `37703e6` and #262's `39e6f5b`), which is also the promoted build. This is the current plan for
...
| Pane search | The find bar ⌘F opens over a SHELL pane becomes the sixth presented surface, `pane.search`, selected in Settings → Plugins → Workbench views with the native bar named "(bundled)" as the recovery floor and `ui.selectView` refusing the placement. One frame carries the one pane the daemon is searching - the needle, the case flag, the total, the selected match and the rectangle the bar may occupy - and one view over the grid is clipped to that rectangle at the pane's top-trailing corner, above a presented pane chrome band. The total and the selection stay workspace state on the delta stream, and the needle a bar is handed is the daemon's or, while it is still on its way there, the one the user typed last, so a bar taking over mid-word loses nothing; a presenter asks through six calls and holds none of it. Scrollback, other panes' state, paths and plugin ids are withheld; the counts are stated as a per-needle read of the searched buffer. A presenter declares its box and the host clamps it to the smaller of 480 px and the pane's inner width and the smaller of 96 px and a quarter of the pane. Four chords are relayed (Escape, the toggle-search chord, ⌘G, ⇧⌘G, with the Ctrl forms where Ctrl is primary), ⌘G steps only while the frame or the searched pane holds the caret, and everything else stays in the frame; the caret goes back to the pane when the SESSION ends and not when the presenter stands down. Markdown and diff previews, web panes and the phone keep their native bars. On failure the native bar returns with the needle intact and its field focused and the case flag back off, with a toast, a `pane.search` status row and **Retry presenter**. [Search Lab](../examples/plugins/search-lab) is the build-free example. | [#262](https://github.com/benfriebe/kelpi/pull/262) | [Pane search](plugin-ui.md#pane-search), [validation](plugin-validation.md#selectable-search-presenter-2026-09-20) |
| Plugin commands in Help | Help stays host-drawn and is not a placement. It now lists every command of each enabled, running plugin under the plugin's display name, whatever its `when` says, with the chord that runs it after the user's overrides; a shortcut a built-in or an earlier plugin claims first is shown with what runs instead. Both halves come from one read-only keyboard model built on the dispatcher's own chord resolution, and the chrome snapshot publishes it as `keymap`, bounded to 64 KiB, so a view reads the map Help draws and cannot rebind through it. | [#261](https://github.com/benfriebe/kelpi/pull/261) | [Keyboard map](plugin-chrome.md#the-keyboard-map), [validation](plugin-validation.md#help-lists-plugin-commands-and-shortcuts-2026-09-24) |
| Root layout | The toolbar (`topbar`), status bar and bottom panel can be hidden beside the Workspaces and Inspector sidebars, from the View menu, the palette, the chrome commands and Settings; hiding is a layout flag, so the selected view comes back and a hidden plugin view stays loaded with `visible=false`. **Zen Mode** (`toggle_zen_mode`, ⌃⌘Return on macOS) hides all five and restores exactly what it recorded. While the toolbar is hidden the host draws an 8 px strip that is the window's drag region and carries a restore handle no plugin can hide, and the shell hides the macOS traffic lights through a `window-chrome` report. A view in a band declares its height statically in the manifest (`bandHeights`, validated at install). The arrangement, sidebar and Inspector visibility included, is persisted per window in `kelpi.workbench.layout.v1`; Zen Mode and Reset Window Arrangement join the recovery floor. | [#263](https://github.com/benfriebe/kelpi/pull/263) | [Hiding bands and Zen Mode](plugins.md#hiding-bands-and-zen-mode), [validation](plugin-validation.md#root-layout-hidden-bands-zen-mode-and-band-heights-2026-09-24) |

Fourteen PRs merged between #244 and #262, and only one of them is a plugin contract: #253, which
has its row above. The rest are product and audit work with no row here and no dated section in the
validation record, named so everything up to `225ef73`, the commit #262 was based on, is accounted
for:
...
behind it. Root layout adds no placement and no `ui.*` call: an optional manifest field
...
| Remaining UI composition, phased | Audited against a concrete replacement example per surface (the audit memo behind pane chrome phase A). **Pane chrome is done**: phase A ([#243](https://github.com/benfriebe/kelpi/pull/243)) shipped the shared model, height authority, parking, projection and SDK types, and phase B ([#244](https://github.com/benfriebe/kelpi/pull/244)) shipped the selectable presenter, one frame over the grid clipped per pane, the two watchdogs and the 240-call budget, the all-or-nothing fallback with its toast and Retry, and the Pane Lab example. **Search is done** ([#262](https://github.com/benfriebe/kelpi/pull/262), the completed table's `pane.search` row), reusing pane chrome's frame-over-the-grid plumbing rather than a second copy of it. **Help's plugin commands and shortcuts are done** ([#261](https://github.com/benfriebe/kelpi/pull/261), the completed table's "Plugin commands in Help" row): the keybinding map, plugin shortcuts included, is a read-only `keymap` in the chrome snapshot and the native overlay draws the same model - not a placement. **Root layout is done** ([#263](https://github.com/benfriebe/kelpi/pull/263), the completed table's "Root layout" row): the toolbar, status bar and bottom panel can be hidden beside the two sidebars, **Zen Mode** (⌃⌘Return) hides all five and restores exactly what it hid, a host-drawn 8 px strip replaces a hidden toolbar as the drag region and the way back, plugin views declare static `bandHeights`, and the arrangement is persisted per window beside the workbench store. | Nothing in this row remains open. Two surfaces are decided against and are **not APIs**: the **phone shell** stays bundled until physical-device coverage exists, because the keyboard inset and the visible-pane report are correctness rather than presentation; and the **Help overlay's frame** stays host-drawn, because replacing it buys a plugin nothing it cannot already draw in a pane. |
...
[merge and promote of #262, #261 and #263](plugin-validation.md#merge-and-promote-of-262-261-and-263-2026-09-24),
[root layout](plugin-validation.md#root-layout-hidden-bands-zen-mode-and-band-heights-2026-09-24),
[plugin commands in Help](plugin-validation.md#help-lists-plugin-commands-and-shortcuts-2026-09-24),
...
the [search record](plugin-validation.md#selectable-search-presenter-2026-09-20) has both. The Help
branch's battery passed unretried in 25.6 minutes at `6fde921`, before its rebase onto `39e6f5b`:
root tests **8,677 passed, 1 skipped**, all scenarios hidden in 6.4 min, the full audit with **1
failed** of 1,716 assertions (`agent-lifecycle` AGNT-055, 3/3 alone on the same revision and
attributed to load) and a packaged smoke of **69/69**; the
[Help record](plugin-validation.md#help-lists-plugin-commands-and-shortcuts-2026-09-24) has it. The
root layout branch's battery passed unretried in 25.9 minutes at `e7d9de1`, before its rebase onto
`c32cabe`: root tests **8,697 passed, 1 skipped**, all scenarios hidden in 6.4 min, the full audit
with **0 failed** of 1,716 assertions and a packaged smoke of **69/69**. Its first battery, at
`819da74`, failed on one scenario's synchronization (`plugin-workbench`) and is kept as history in the
[root layout record](plugin-validation.md#root-layout-hidden-bands-zen-mode-and-band-heights-2026-09-24).

**Main after the three merges has no battery of its own.** The last battery on `main` itself is the
integration run at `225ef73` that preceded the 2026-09-23 promote: passed in 25.9 minutes, root tests
**8,651 passed, 1 skipped**, shell **882**, the full audit with **0 failed** of 1,716 assertions and
a packaged smoke of **69/69**. #262, #261 and #263 merged on 2026-09-24, each merge commit's tree
identical to the head its evidence was taken on. The three together were tested as a local combined
build (typecheck, root **8,838 passed, 1 skipped**, shell **890**, and the seven scenarios the
branches touch, all green) and by the owner by hand, and `058c8f4` was promoted the same day at the
owner's request **without** an integration battery, which was started and stopped mid-run. The next
battery on `main` is owed; the
