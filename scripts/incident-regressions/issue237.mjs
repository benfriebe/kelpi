...
function buildExtraCleanupPlan({ retain, extraDaemons, extraSandboxes, exists, fs, cleanup, clearStack }) {
    const states = extraDaemons.map(() => 'unknown');
    return [
        ['retain remote daemon output', () => retain('remote-daemon-output.json',extraDaemons.map(p=>({pid:p.child?.pid,text:p.text(),exited:p.exited})), 'runtime')],
        ...extraDaemons.map((daemon, index) => [`stop remote daemon ${index}`, () => daemon.stop()]),
        ...extraDaemons.map((daemon, index) => [`check remote daemon process ${index}`, () => observeOwnedProcess({name:'remote-daemon',pid:daemon.child?.pid,index,states,exists,cleanup})]),
        ...extraSandboxes.map((sandbox, index) => [`cleanup remote sandbox ${index}`, () => {if(allOwnedProcessesProvenGone(states))sandbox.cleanup();}]),
        ...extraSandboxes.map((sandbox, index) => [`check remote sandbox ${index}`, () => {if(fs.existsSync(sandbox.root))cleanup.leaks.push({name:'remote-sandbox-left-by-scenario',path:sandbox.root,processStates:[...states]});}]),
        ['clear incident237 stack binding', clearStack],
    ];
}
function buildReorderScenarioCleanupPlan({ observer, fs, sandbox, previousConfig, d, page, host, daemon, remote, exists, cleanup }) {
    const states = ['unknown'];
    return [
        ['close reorder observer', () => observer?.ws.close()],
        ['restore primary sandbox config', () => fs.writeFileSync(sandbox.configPath, previousConfig)],
        ['wait for remote host removal', () => d.settleDom(page, `!document.querySelector('${host}')`, { ceilingMs: 10_000 })],
        ['stop reorder remote daemon', () => daemon?.stop()],
        ['check reorder remote daemon process', () => observeOwnedProcess({name:'reorder-remote-daemon',pid:daemon?.child?.pid,index:0,states,exists,cleanup})],
        ['cleanup reorder remote sandbox', () => {if(remote && allOwnedProcessesProvenGone(states))remote.cleanup();}],
        ['check reorder remote sandbox', () => {if(remote && fs.existsSync(remote.root))cleanup.leaks.push({name:'reorder-remote-sandbox',path:remote.root,processStates:[...states]});}],
    ];
}
...
try {
    ...
    try {
        ...
        await runCleanupPlan(buildExtraCleanupPlan({
            retain, extraDaemons, extraSandboxes, exists, fs, cleanup,
            clearStack: () => {delete globalThis.__incident237Stack;},
...
async function incident ({ page, cli, sandbox, rec, d, sleep, repoRoot, harness }) {
    ...
    try {
        ...
        // These cancellation controls bind the actual pre-gesture state so that the
        // original ref's failed positive reorder cannot turn cleanup into a false error.
        const stableTop=[...topIDs()], stableChildren=[...children(group)];
        const stable=()=>same(topIDs(),stableTop) && same(children(group),stableChildren);
        let end=await begin(first,tail);
        const hostBox=await page.box(host);
        await release({x:hostBox.x+hostBox.width+30,y:end.y});await sleep(350);
        rec.check('release outside the remote host preserves both sibling orders',stable());
        end=await begin(first,tail);
        const localRow=await page.box('[data-testid="workspace-row"]');
        await release({x:localRow.cx,y:localRow.cy});await sleep(350);
        rec.check('release on the local host cannot mutate remote workspace order',stable());
        end=await begin(first,tail);
        const groupRow=await box(memberB);
        await release({x:groupRow.cx,y:groupRow.cy});await sleep(350);
        rec.check('cross-container release neither reorders nor reparents a workspace',stable());
        end=await begin(first,tail);
        await page.key('Escape');await release(end);await sleep(350);
        rec.check('Escape cancels the in-progress remote drag',stable());
        end=await begin(memberA,memberB);
        await remoteCLI.ok(['workspace','move',memberA,'--top-level']);
        rec.check('an independent client membership change reaches the active drag',await d.settle(async()=>!children(group).includes(memberA) && topIDs().includes(memberA) && (await domIDs()).at(-1)===memberA));
        await release(end);await sleep(350);
        rec.check('a stale gesture cannot return the externally moved workspace to its old group',!children(group).includes(memberA) && topIDs().includes(memberA));
        await remoteCLI.ok(['workspace','move',memberA,'--group',group,'--index',String(stableChildren.indexOf(memberA))]);
        if(!await d.settle(stable))throw new Error('Independent-client restore did not echo the exact pre-control state');
        rec.check('the remote observer received a gap-free state stream',observer.error===null,observer.error??'');
        rec.check('drag input reached the renderer as trusted pointer events',await page.eval('window.__reorderTrusted.length>=7 && window.__reorderTrusted.every(Boolean)'));

        // Exercise unchanged local drag positively, rather than only proving remote
        // operations leave the local list untouched.
        const localFirst=JSON.parse(await cli.ok(['workspace','create','--name','Local reorder control first','--json'])).workspace_id;
        const localLast=JSON.parse(await cli.ok(['workspace','create','--name','Local reorder control last','--json'])).workspace_id;
        const localToken=fs.readFileSync(path.join(sandbox.runDir,`daemon-v${PROTOCOL_VERSION}.token`),'utf8').trim();
        const localObserver=await observe(sandbox,localToken);
        try{
            const initialLocal=localObserver.top.map(entry=>entry.id);
            const aSelector=`[data-testid="workspace-row"][data-workspace-id="${localFirst}"]`;
            const bSelector=`[data-testid="workspace-row"][data-workspace-id="${localLast}"]`;
            if(!await d.settleDom(page,`document.querySelector(${JSON.stringify(aSelector)}) && document.querySelector(${JSON.stringify(bSelector)})`))throw new Error('Local drag control fixture not rendered');
            const a=await page.box(aSelector),b=await page.box(bSelector);
            await page.drag(b.cx,b.cy,a.cx,a.y+a.height/4);
            const expectedLocal=initialLocal.filter(id=>id!==localLast);expectedLocal.splice(expectedLocal.indexOf(localFirst),0,localLast);
            rec.check('local workspaces still reorder through the native sidebar gesture',await d.settle(()=>same(localObserver.top.map(entry=>entry.id),expectedLocal)));
            rec.check('local drag does not alter the remote order',stable());
        }finally{localObserver.ws.close();}
        observer.ws.close();
        ...
        await runCleanupPlan(buildReorderScenarioCleanupPlan({
            observer, fs, sandbox, previousConfig, d, page, host, daemon, remote, exists, cleanup,
        }), recordCleanupFailure);
    }
}
