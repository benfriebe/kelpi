...
The last five phases added the placements `interaction.palette`, `interaction.prompts`,
`interaction.notifications`, `settings.window` and `pane.chrome`, one presenter call
(`ui.setNotificationBoxHeight`), one declaration-only type module (`pane-chrome.d.ts`), one further
container refusal, and two additive terminal SDK fields. All are additive: plugin API version remains **1** and wire protocol
generation remains **2**. `pane.chrome` is the one placement with no host behind it yet: it
validates in a manifest, has no bundled default and no Settings row, and phase B is what makes it
selectable.
