...
export const STEP_MANIFEST = [
    ...
    { id: 'phone-keyboard-inset', lane: 'phone', chain: null, cost: 8.5, reason: 'C2\'s live half: it emulates the same 390x844 phone, shadows `height` on the live VisualViewport instance to fake a 300 px software keyboard, and reads the rows and the resize count back off every terminal pane. It provisions nothing, reads no shared `state`, moves no focus and types nothing; the only thing it changes is the size of the panes already on screen, which is what `emulatePhone` does on its own and what `clearPhoneEmulation` undoes - and both the shadow and the emulation are removed in a `finally`. Measured in a three-step run (`--only fresh-boot,terminal-ls,phone-keyboard-inset`), the `state-timeline.jsonl` rows either side of it are identical. Placed next to `phone-form-factor` so the phone lane stays contiguous at the tail of the canonical order' },
    { id: 'phone-key-bar', lane: 'phone', chain: null, cost: 22.0, reason: 'C1\'s live half: it emulates the phone, taps the key bar\'s Ctrl with a real touch, types c, and reads the interrupt back through `pane capture`. It provisions nothing and moves no pane, workspace or setting - it borrows the widest shell pane already on screen, focuses it (which the spine\'s own steps do all run long) and leaves it back at a prompt, so the `state-timeline.jsonl` rows either side of it are identical. The emulation is cleared in a `finally`. Placed next to `phone-form-factor`, immediately before `mac-chrome` (which must stay last), so the phone lane stays one contiguous block at the tail' },
    { id: 'phone-paste', lane: 'phone', chain: null, cost: 26.0, reason: 'C4\'s live half: it taps the key bar\'s Paste with a real touch and reads the bracketed-paste envelope back through `pane capture`, both with DEC 2004 set and with it cleared. Same contract as `phone-key-bar` - it borrows the widest shell pane already on screen, restores DEC 2004 and the prompt before it returns, provisions nothing, moves no setting, and clears the emulation in a `finally`. It deliberately does not drive the OSC 52 Copy pill, which would need `clipboard-write = true` (a setting, which a phone step may not move); that surface is unit-tested and on the owner device checklist. Placed with the rest of the phone lane, before `mac-chrome`' },
    { id: 'phone-touch-scroll', lane: 'phone', chain: null, cost: 14.0, reason: 'C3\'s live half: it emulates the same 390x844 phone, drags and flicks the terminal with real CDP touch, reads the viewport back off `data-terminal-scroll` and the word off `data-terminal-selection`, and proves through `pane capture` that none of it reached the PTY. Same contract as `phone-key-bar` - it borrows the widest shell pane already on screen, provisions nothing, moves no workspace, pane or setting, and leaves the pane at a prompt with its viewport at the bottom, no selection and no pill; the emulation is cleared in a `finally`. What it leaves behind is 300 lines of scrollback in a pane `mac-chrome` deletes anyway, which is what `phone-key-bar` leaves too. Placed with the rest of the phone lane, after `phone-paste`, so the lane stays one contiguous block before `mac-chrome`' },
    { id: 'phone-touch-mouse-reporting', lane: 'phone', chain: null, cost: 26.0, reason: '#123\'s live half, and the half `phone-touch-scroll` cannot be: that step asserts `data-terminal-mouse=none` before it drags, so the mode the owner\'s defect lives in was never driven by a finger. This one runs `fixtures/mouse-report-probe.mjs` in the borrowed pane - the alternate screen plus `?1000h ?1002h ?1006h`, which is what Claude Code asks for - drags, flicks, taps and long-presses with real CDP touch, and reads the byte trail out of the fixture\'s log rather than out of the pane, because an alternate screen has no rows `pane capture` can show. Same contract as `phone-touch-scroll`, whose pane it borrows: it takes the widest shell pane already on screen, provisions nothing, moves no workspace, pane or setting, starts and stops its own fixture through `pane send` / `pane send-key` (never through the caret, which under mouse reporting is not reachable by a click), and clears the emulation in a `finally`. It adds NOTHING to the pane\'s scrollback - the fixture paints on the alternate screen - and leaves it at a prompt with `data-terminal-mouse=none`, which the step asserts. Placed immediately after `phone-touch-scroll`, the step it is the other mode of, so the phone lane stays one contiguous block before `mac-chrome` (which must stay last)' },
    { id: 'phone-caret-owner', lane: 'phone', chain: null, cost: 30.0, reason: 'C5\'s live half: it emulates the same 390x844 phone, taps the key bar with the keyboard down and then reproduces the focus move Android makes onto the tapped button (`button.focus()`, which CDP\'s own touch does not make), and opens and closes Settings. Same contract as `phone-key-bar`, whose pane it borrows - it takes the widest shell pane already on screen, drives only keys that change neither the shell\'s line nor its history (`esc`, `home`, `end`, `left`, and `ctrl` twice so the latch is spent on itself), leaves the pane at a prompt with the caret back on the engine, provisions nothing, writes no setting, creates and destroys no pane or workspace, reads the roster either side and asserts it is byte-identical, and clears the emulation in a `finally`. Placed immediately after `phone-paste` so the phone lane stays one contiguous block before `mac-chrome` (which must stay last)' },
    { id: 'phone-key-bar-split', lane: 'phone', chain: null, cost: 34.0, reason: 'C9\'s live half: it emulates the same 390x844 phone with the terminal SPLIT in two, and measures the one thing a single pane cannot show - that there is exactly one key bar, that it spans the content area rather than the pane it acts on, and that the pane a thumb taps is the pane the bar then interrupts. It is the one phone step that PROVISIONS: it splits a shell pane and closes it again in the `finally`, reads the pane roster either side and asserts it is byte-identical, leaves both shells at a prompt (each `sleep 30` is interrupted by the bar itself), moves no workspace and writes no setting, and clears both the visual-viewport shadow and the emulation in that same `finally`. Placed immediately after `phone-caret-owner` so the phone lane stays one contiguous block before `mac-chrome` (which must stay last)' },
    { id: 'phone-settings-sheet', lane: 'phone', chain: null, cost: 9.0, reason: 'B5\'s live half for Settings: it emulates the same 390x844 phone, opens the overlay with ⌘, and taps through the sheet\'s two screens (the tab list, one pushed tab, back, Close). Settings is a modal over daemon state and this step only NAVIGATES it - it writes no setting, provisions nothing, creates and destroys no pane or workspace, and it reads the pane roster, the focused pane and the active workspace either side of itself and asserts they are byte-identical (the check is in the step, so a regression reports itself rather than being argued). The emulation is cleared in a `finally` and the sheet with it. Placed next to the other phone steps so the lane stays contiguous at the tail of the canonical order' },
    { id: 'phone-palette-sheet', lane: 'phone', chain: null, cost: 9.5, reason: 'B5\'s live half for the command palette: it emulates the same 390x844 phone, opens the palette on its live binding, shadows `height` on the live VisualViewport instance to fake a 300 px software keyboard (C2\'s seam, same shape as `phone-keyboard-inset`), types a query and confirms ONE row - deliberately the pane that already has focus, so the confirm\'s only effect (a focus change plus the §10.4 handoff) lands where focus already was. Like the settings step it reads the roster, the focused pane and the active workspace either side and asserts they are identical; the shadow and the emulation are both removed in a `finally`. Placed next to the other phone steps so the lane stays contiguous at the tail of the canonical order' },
    { id: 'phone-shell', lane: 'phone', chain: null, cost: 16.0, reason: 'B1/B2/B3\'s live half: it emulates the same 390x844 phone and drives the phone shell - the header, the layout toggle both ways, the workspace drawer by tap, and the pane sheet. Same contract as `phone-key-bar`: it borrows the widest shell pane already on screen and focuses it at desktop size, provisions nothing, writes no setting (the view mode it toggles is toggled back, so the remembered choice is what it was), creates and destroys no pane or workspace, moves focus to a sibling and back through the sheet so the roster read either side is byte-identical, and clears the emulation in a `finally`. Placed with the rest of the phone lane, after `phone-palette-sheet`, so the lane stays one contiguous block before `mac-chrome` (which must stay last)' },
    { id: 'phone-view-toggle', lane: 'phone', chain: null, cost: 18.0, reason: 'B8\'s live half (issue #120): it emulates the same 390x844 phone and taps the view toggle both ways with a RUNNING pane, sampling `data-terminal-status`, `data-terminal-resizes` and the pane\'s canvas every 25 ms so "the pane kept its terminal" is a measurement rather than a screenshot, and it runs a raw reader (`stty -icanon -echo min 1; cat -v`) through a second round trip so "the toggle typed nothing into the PTY" is one too. Same contract as `phone-shell`, whose pane it borrows: it takes the widest shell pane already on screen, provisions nothing, writes no setting (the view mode it toggles is toggled back, so the remembered choice is what it was), creates and destroys no pane or workspace, restores the line discipline and leaves the pane at a prompt, reads the roster either side and asserts it is byte-identical, and clears the emulation in a `finally`. What it leaves behind is a screen of ruler lines in a pane `mac-chrome` deletes anyway, which is what `phone-key-bar` leaves too. Placed immediately after `phone-shell` so the phone lane stays one contiguous block before `mac-chrome` (which must stay last)' },
    { id: 'phone-landing', lane: 'phone', chain: null, cost: 22.0, reason: 'B7\'s live half: it emulates the same 390x844 phone with the remembered place CLEARED, so it sees the first open the other phone steps seed past, and drives the landing page (host cards, the origin\'s workspaces, the workspace that leaves it) and every pane type through the shell (a markdown pane through the desktop\'s own preview, a web pane as the phone card, the key bar on for the terminal and off for both). It PROVISIONS like `phone-key-bar-split`: it opens one markdown pane and one web pane and closes both in the `finally`, reads the pane roster either side and asserts it is byte-identical, moves no workspace and writes no setting beyond the phone\'s own `kelpi.phone.last-place`, which the shared `clearPhoneEmulation` removes. Placed immediately after `phone-shell` so the phone lane stays one contiguous block before `mac-chrome` (which must stay last)' },
