...
| verb | args | host must |
|---|---|---|
| `pane-open` | `{paneID, isPrivate, activeTabID, tabs:[{id,url,title}]}` | create/refresh the pane's view set and its storage partition (persistent, or an in-memory one when `isPrivate`); load each tab's URL. Idempotent, sent again on every host registration **and as the rebuild after a renderer death** (below). |
...
| `blur-view` | `{}` | The inverse (issue #33): hand the window's keyboard back to the CLIENT's renderer (`window.webContents.focus()`). `{ok:true, restored}` - `restored` is false when this window is not the active one, which is not an error. Takes no tab: the destination is the window, not a page. The client sends it when its ring leaves every web pane in the window, because a native view keeps the window's keyboard until something takes it, and `focusPaneSurface` is DOM focus, which cannot outrank a sibling widget. With the mouse the press itself makes the renderer first responder and this is never needed; from the keyboard (⌘[ / ⌥⌘←) nothing does it, and the ring landed on a pane that would not type. GUI-only (`web-blur-view`). |
...
| `page-state` | `{url?, title?}` | Mirrors into the tab record and the pane header. `""`/`about:blank` URLs are ignored as placeholders (§4.4); titles are always taken. A `url`-carrying event also re-applies an open find and re-pushes the batch markers against the new DOM (§10, §12). Send on `did-navigate`, `did-navigate-in-page`, `page-title-updated`, and `{url}` again on `did-finish-load` (not for the error card): at commit the document may have no body yet, so the re-apply has to run once the load is done (issue #50). |
...
| `tab-closed` | `{}` with `tabID` | The host closed a tab on its own (`window.close()`, a crash): the daemon drops it from the tab list and re-activates the left neighbour. **When it was the pane's ONLY tab** the tab list refuses to shrink, and the daemon answers the refusal instead of ignoring it (web-pane.md §5.2, issue #76): the first such death in 30 s is answered with a fresh `pane-open` for that pane (the rebuild, which the host reconciles by creating a view for the tab it no longer holds), and a second one inside the window is not, because a page that crashes as it loads would otherwise spin the host for ever. Send this event whenever a tab's renderer dies, and destroy the view before or as you send it: a `WebContentsView` whose renderer is gone is still parented and still covering the pane. |
