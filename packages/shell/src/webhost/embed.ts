...
export function createEmbedController<V>(options: EmbedOptions<V>): EmbedController<V> {
     ...
     * reconnect and closed for the rest of the process's life.
     */
    ...
    return {
        apply(geometry) {
            ...
            // #72: a report that reaches this line is a client speaking about that pane, which
            // is the only thing that can confirm a placement the host is holding across a
            // reconnect. Whatever the report then decides (place, park, release), the CLAIM has
            // been re-stated by the party that owns it.
            unconfirmed.delete(geometry.paneID);
