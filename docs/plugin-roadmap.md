...
| Full Settings presentation | **All three phases implemented on `feature/plugin-settings-contracts`, `feature/plugin-settings-presenter` and `feature/plugin-settings-lab`; the row moves to the completed table once the PRs merge.** A shared settings model (closure-free section and field descriptors, host-owned validation, drafts and routing) renders General, Workspaces and Appearance's plain rows, and a plugin view declaring `settings.window` can be selected as the Settings presenter with the bundled panel as the recovery floor. Plugins, Remote, Profiles, Keybindings, Labels, Repositories, Web, both key recorders, Appearance's theme importer and every destructive confirmation stay native. | Remaining: an Appearance presenter for the native remainder parts (theme gallery, importer, colour maps) if a real plugin needs them; selectable notification presentation; physical-device checks. |
