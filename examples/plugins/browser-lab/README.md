...
To try it without replacing your installed Kelpi, run a private development instance from this checkout:

```sh
node scripts/dev-instance.mjs --state out/browser-plugin-playground
```

In another terminal, use the `KELPI_SOCKET` value printed by that instance and the CLI built from the same checkout:
