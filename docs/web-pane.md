...
- On provisional or committed navigation failure: build a small self-contained dark error card (no external assets): "Couldn't load page", the failed URL (monospace, blue), the error message, and a **Retry** anchor whose `href` is the failed URL (clicking re-navigates through the normal pipeline). HTML-escape `& < > "` in all three interpolations. `failedURL = lastAttemptedURL ?? currentURL ?? ""`; display "(unknown)" when empty, message fallback "The page could not be loaded.". The stub is loaded with **baseURL = the failed URL** so the engine's reported URL stays on what the user tried (URL bar doesn't flip to about:blank). (The port has no `baseURL`: the card is a `data:` URL, and the tab's `url()` reports the failed address instead while the card is up or a failure is pending, `packages/shell/src/webhost/error-page.ts` `reportedTabURL`, so `web-url`, `web-capture` and the console line never see the card's own URL; the address is taken from the failure itself, so a failed link click or redirect reports and retries where it actually failed.)
...
- Chord forwarding: keystrokes land in the page's renderer first, so the shell intercepts every chord Kelpi claims in `before-input-event` (key-downs only, matched by physical `code`), cancels them in the page and replays them into the Kelpi window as `web-chord:<code>[:<modifier>]*` menu commands (`packages/shell/src/webhost/keys.ts`, `tab.ts`). The claimed set is DERIVED from the resolved binding map rather than listed, so a rebound action moves with the rebinding and a chord Kelpi does not claim is never taken from the page (issue #33); the §7.3 priority table and ⌘, / ⌘? are folded in because neither is in the map. A chord must hold ⌘, ⌃ or ⌥ to be taken at all - shift alone is how a page's user types a capital, and forwarding the map's one unmodified line (`escape=close_search`) would steal Escape from every page. The client replays each as a real `keydown`, so the palette guard, the priority layer below and the normal binding lookup all run in order (`packages/client/src/webpane/priority.ts`; ⌘F reaches `toggle_search` this way). Bare ⌘[ / ⌘] are forwarded like any other binding and mean focus-previous/next-pane, per §7.3's issue #229 decision - they are not page back/forward, and nothing in an embedded `WebContentsView` implements them as such.
