...
export function createSyncHub(options: SyncHubOptions): SyncHub {
    ...
    return {
        ...
        broadcast(event) {
            ...
            // window session hears it, so no banner, sound, bounce or title flash (§7.6). Scoped
            // to those two types, so an unrelated broadcast that ever carries `muted` still flows.
            if ((type === 'notification' || type === 'attention-request') && message['muted'] === true) return;
