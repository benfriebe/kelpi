...
function buildPanCleanupPlan({ page, observer, sleep, cli, initialWorkspaceIDs, startingWorkspace, d, recordFailure }) {
    let listedWorkspaces = [];
    return [
        ['clear device metrics override', () => page.send('Emulation.clearDeviceMetricsOverride')],
        ['close owner observer', () => observer.close()],
        ['settle after owner observer close', () => sleep(200)],
        ['list workspaces opened by pan scenario', async () => {listedWorkspaces = JSON.parse(await cli.ok(['workspace', 'list', '--json']));}],
        ['delete every workspace opened by pan scenario', () => runCleanupPlan(
            listedWorkspaces.filter(workspace => !initialWorkspaceIDs.has(workspace.id))
                .map(workspace => [`delete pan workspace ${workspace.id}`, () => cli.run(['workspace', 'delete', workspace.id, '--force'])]),
            recordFailure,
        )],
        ['restore starting workspace selection', async () => {
...
async function incident ({ page, cli, sandbox, rec, d, sleep }) {
    ...
    try {
        ...
        await runCleanupPlan(buildPanCleanupPlan({
            page, observer, sleep, cli, initialWorkspaceIDs, startingWorkspace, d,
