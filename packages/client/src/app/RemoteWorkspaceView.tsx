...
export function RemoteWorkspaceView(props: RemoteWorkspaceViewProps): ReactElement {
    ...
    // One `pane.header` resolution per pane per render, read as chips and as descriptors.
    const paneHeaderItems = usePaneHeaderContributions(contributions.items);
    ...
    return (
        <PaneGrid
            ...
            headerExtras={paneID => {
                const items = paneHeaderItems.items(paneID);
            ...
            /* The projection half of the same resolution, and the run reached by id rather than by
