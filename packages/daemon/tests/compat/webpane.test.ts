...
describe.skipIf(!swiftCLIAvailable())('compat: web panes with a live host', () => {
    ...
    beforeEach(async () => {
        ...
        shell = await connectFakeHost(kelpi.info);
    ...
    it('round-trips an actuator verb and prints the host result', async () => {
        const running = kelpi.run(['web', 'text', '--target', paneID, 'css:body'], { timeoutMs: 15_000 });
    ...
    it('keeps `exists` exit-code semantics (0 = found, 1 = missing)', async () => {
        const yes = kelpi.run(['web', 'exists', '--target', paneID, 'css:#login'], { timeoutMs: 15_000 });
        await shell.answer({ ok: true, found: true }, 'actuate');
        expect((await yes).code).toBe(0);

        const no = kelpi.run(['web', 'exists', '--target', paneID, 'css:#nope'], { timeoutMs: 15_000 });
    ...
    it('keeps `attr` exit 1 for an absent attribute, exit 0 for an empty one', async () => {
        const absent = kelpi.run(['web', 'attr', '--target', paneID, 'css:#a', 'disabled'], {
            timeoutMs: 15_000
        });
        await shell.answer(
        ...
        const empty = kelpi.run(['web', 'attr', '--target', paneID, 'css:#a', 'disabled'], {
            timeoutMs: 15_000
        });
        await shell.answer(
    ...
    it('drains the console buffer the host filled, honouring --since and --level', async () => {
        ...
        const all = await kelpi.run(['web', 'console', '--target', paneID], { timeoutMs: 15_000 });
        ...
        const errorsOnly = await kelpi.run(
        ...
        const since = await kelpi.run(['web', 'console', '--target', paneID, '--since', '1'], {
            timeoutMs: 15_000
        });
