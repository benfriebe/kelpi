...
/**
 * Dev-server proxy target: the daemon's **HTTP/WS** port (`NEXD_HTTP_PORT`), not its control
 ...
 * `NEX_DAEMON_URL` when the daemon took an ephemeral port; the alternative (and the one that
 * also works from another machine) is the client's own `?daemon=` override, which bypasses
 * the proxy entirely. See README.md.
 */
const daemonUrl = process.env['NEX_DAEMON_URL'] ?? 'http://127.0.0.1:19470';
