...
function isFullDocument(source: string): boolean {
    const prolog = /[ \t\r\n\f]+|<!--[\s\S]*?-->/y;
    let start = 0;
    while (prolog.exec(source)) start = prolog.lastIndex;
    return /^<(?:!doctype[ \t\r\n\f]+html|html|head)(?=[ \t\r\n\f/>])/i.test(source.slice(start));
}
...
export function pluginDocument(html: string, assetRoot: string, entry: string, configuration: JsonObject): string {
    ...
    // Explicit documents keep their head open so authored styles survive body replacement.
    // Fragments need a body before even a leading synchronous script runs. Detect only a
    // leading document marker; leave parsing to the frame so no resources load in the host.
    const authored = isFullDocument(source) ? source : `</head><body>${source}</body>`;
    return `<!doctype html><html><head>${injection}${authored}</html>`;
}
