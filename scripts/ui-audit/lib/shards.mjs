...
export const STEP_MANIFEST = [
    ...
    { id: 'settings-empty-centering', lane: 'spine', chain: null, cost: 4.0, reason: 'opens Settings itself and measures the Labels and Web empty states against the panel; but it CLEARS any label preset or favourite it finds and applies-then-clears a workspace label to raise the adoption section, so it must not run beside a step that is still using either' },
