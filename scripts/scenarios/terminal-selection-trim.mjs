...
export default async function ({ page, cli, sandbox, rec, d, sleep, repoRoot }) {
    const { Ghostty } = await import(pathToFileURL(path.join(repoRoot, 'vendor/ghostty-web-patched/dist/ghostty-web.js')));
    ...
    try {
        ...
        const module = await WebAssembly.compile(fs.readFileSync(path.join(repoRoot, 'vendor/ghostty-web-patched/ghostty-vt.wasm')));
