...
SDK; see the [browser guide](plugin-browser.md). The command palette, the shared prompts and the
notification stack are registered the same way, as the bundled `kelpi.palette`, `kelpi.prompts`
and `kelpi.interaction.notifications` features behind the `interaction.palette`,
`interaction.prompts` and `interaction.notifications` placements; the window interaction host
mounts them, so the single modal registration, focus authority and Escape policy stay host-side
whoever draws. A selected prompts presenter draws modal requests only, and the corner stack is a
placement of its own with its own selection and its own host-drawn box.
See [selectable interaction presenters](plugin-ui.md#selectable-interaction-presenters).
