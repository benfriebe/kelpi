...
export class Terminal implements ITerminalCore {
  ...
  // loop can decide in four comparisons whether any DOM write is needed at all.
  private imeCaretLeft = -1;
  private imeCaretTop = -1;
  private imeCaretWidth = 0;
  private imeCaretHeight = 0;
  /** In-flight composition text; '' means no composition is active. */
  private imePreeditText = '';
...
// Note: We intentionally do NOT set contenteditable on the parent container.
      ...
      // making it pointer-transparent costs nothing.
      ...
      // the thick underline macOS draws under uncommitted text. Position is applied by
      // updatePreedit() from the same caret geometry the textarea uses, so the two can never
      // point at different cells.
      this.compositionPreview = document.createElement('div');
      this.compositionPreview.setAttribute('data-ime-preedit', '');
      this.compositionPreview.setAttribute('aria-live', 'polite');
      this.compositionPreview.style.position = 'absolute';
      this.compositionPreview.style.left = '0';
      this.compositionPreview.style.top = '0';
      // `auto`, explicitly: an embedder stylesheet that sizes the container's children (Nex
      // stretches `[data-terminal-host] > *` to 100%) would otherwise stretch the preedit into
      // a full-pane block and its measured origin would stop meaning "the caret".
      this.compositionPreview.style.width = 'auto';
      this.compositionPreview.style.height = 'auto';
      this.compositionPreview.style.padding = '0';
      this.compositionPreview.style.margin = '0';
      this.compositionPreview.style.whiteSpace = 'pre';
      this.compositionPreview.style.pointerEvents = 'none';
      ...
      // Composition lifecycle → preedit text. `compositionend` ALWAYS clears, whether the IME
      // committed or the user cancelled, and so does a blur: a preedit that outlives its
      // composition is worse than no preedit at all, because it lies about the cell.
      this.textarea.addEventListener('compositionstart', (e: CompositionEvent) => {
        this.setPreedit(e.data ?? '');
      });
  ...
  private writeInternal(data: string | Uint8Array, callback?: () => void): void {
    // Preserve selection through native tracked pins: absolute JS row indices become
    ...
    try {
      ...
      if (trackedSelection) this.selectionManager!.restoreSelectionAfterWrite();
    }
  ...
  private startRenderLoop(): void {
    const loop = () => {
      if (!this.isDisposed && this.isOpen) {
        ...
        this.syncImeCaret(cursor.x, cursor.y);
  ...
  // ghostty-web 0.4.0 + PR #120 put neither there:
  //
  //   1. The hidden <textarea>. The browser reports the composition's screen rect to the OS
  //      input method from the FOCUSED EDITABLE ELEMENT's layout box, so the candidate window
  //      opens wherever that box is. Left at the container's origin it opens at the pane's
  //      top-left corner no matter where the user is typing.
  //   2. The preedit. Uncommitted text belongs in the cells it is about to occupy.
  //
  // Both are driven from one cached box (imeCaret*), recomputed each frame in syncImeCaret.

  /**
   * Recompute the cursor cell's box and, if it moved or resized, move the IME onto it.
   *
   * Called once per rendered frame with the cursor the renderer just drew.
   */
  private syncImeCaret(cursorX: number, cursorY: number): void {
    const renderer = this.renderer;
    const canvas = this.canvas;
    if (!renderer || !canvas || !this.textarea) return;

    const cellWidth = renderer.charWidth;
    const cellHeight = renderer.charHeight;
    if (!(cellWidth > 0) || !(cellHeight > 0)) return;

    // The cursor is reported in VIEWPORT rows. When the user has scrolled back, the cursor's
    // row is pushed down (and usually off the bottom); clamping to the canvas keeps the
    // candidate window inside the terminal instead of floating over the app around it.
    const canvasWidth = canvas.clientWidth || this.cols * cellWidth;
    const canvasHeight = canvas.clientHeight || this.rows * cellHeight;
    const rawLeft = cursorX * cellWidth;
    const rawTop = (cursorY + this.viewportY) * cellHeight;
    const left =
      canvas.offsetLeft + Math.min(Math.max(rawLeft, 0), Math.max(0, canvasWidth - cellWidth));
    const top =
      canvas.offsetTop + Math.min(Math.max(rawTop, 0), Math.max(0, canvasHeight - cellHeight));

    if (
      left === this.imeCaretLeft &&
      top === this.imeCaretTop &&
      cellWidth === this.imeCaretWidth &&
      cellHeight === this.imeCaretHeight
    ) {
      return;
    }

    this.imeCaretLeft = left;
    this.imeCaretTop = top;
    this.imeCaretWidth = cellWidth;
    this.imeCaretHeight = cellHeight;

    // The textarea is sized to the cell, not to 1×1: an input method positions its candidate
    // window against the caret rect the browser derives from this box, and a 1px box in the
    // corner of a cell reads as the corner of the pane on some IMEs.
  ...
  /**
   * Set (or clear) the in-flight composition text.
   *
   * `''` is the cleared state — compositionend and blur both route here, so the preedit can
   * never outlive the composition that produced it.
   */
  private setPreedit(text: string): void {
    if (text === this.imePreeditText) return;
    this.imePreeditText = text;
    this.updatePreedit();
  }

  /**
   * Paint the preedit overlay at the cached caret box, or hide it when there is no
   * composition. Idempotent: safe to call on a caret move and on a text change alike.
   */
  private updatePreedit(): void {
    const preedit = this.compositionPreview;
    if (!preedit) return;

    if (this.imePreeditText === '') {
      preedit.style.display = 'none';
      preedit.textContent = '';
      return;
    }

    const cellHeight = this.imeCaretHeight > 0 ? this.imeCaretHeight : this.options.fontSize;
    const theme = { ...DEFAULT_THEME, ...(this.options.theme ?? {}) };
    const underline = Math.max(1, Math.round(cellHeight * 0.1));

    preedit.textContent = this.imePreeditText;
    // Marked text, macOS style: the terminal's own font at the terminal's own cell height, on
    // the terminal's background so it covers the cells underneath, under a thick underline.
    preedit.style.font = `${this.options.fontSize}px ${this.options.fontFamily}`;
    preedit.style.lineHeight = `${cellHeight}px`;
    preedit.style.height = `${cellHeight}px`;
    preedit.style.color = theme.foreground;
    preedit.style.backgroundColor = theme.background;
    preedit.style.borderBottom = `${underline}px solid ${theme.foreground}`;
    preedit.style.left = `${Math.max(0, this.imeCaretLeft)}px`;
    preedit.style.top = `${Math.max(0, this.imeCaretTop)}px`;
    preedit.style.display = 'block';

    // Keep a long preedit inside the terminal: with the caret near the right edge the text
    // would otherwise run out of the pane and the tail would be unreadable. Measured after
    // the text is in the DOM, and only ever shifted LEFT, so a preedit at a normal caret
    // position is untouched and still starts exactly on the cursor cell.
    const canvas = this.canvas;
    if (canvas) {
      const available = canvas.offsetLeft + (canvas.clientWidth || 0);
      const overflow = this.imeCaretLeft + preedit.offsetWidth - available;
      if (overflow > 0) {
        preedit.style.left = `${Math.max(0, this.imeCaretLeft - overflow)}px`;
      }
    }
  }
