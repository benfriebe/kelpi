...
export const PLUGIN_PLACEMENTS = ['pane', 'sidebar.primary', 'sidebar.secondary', 'panel.bottom', 'topbar', 'statusbar', 'workspace', 'settings', 'document.markdown', 'document.scratchpad', 'document.diff', 'terminal'] as const;
...
export function decodePluginManifest(raw: unknown): PluginManifest {
    ...
    const containers = array(contributes['containers']).map((raw): PluginContainerDefinition => {
        ...
        if (places.includes('pane') || places.includes('terminal') || places.some(place => place.startsWith('document.'))) throw new Error('containers require workbench placements; pane views own their individual saved state');
