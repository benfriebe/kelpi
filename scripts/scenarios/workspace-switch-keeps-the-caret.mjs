...
/**
 * How many of a workspace's panes will build an engine, which is what `ENGINES_UP` counts.
 *
 * `shell` is the only pane type with a terminal in it (`core/src/layout/pane.ts`: shell, markdown,
 * scratchpad, diff, web, plugin). This used to be `panes.length`, and in the one sandbox the
 * battery shares that is not the same number: the web pane scenarios leave a `web` pane in
 * Default, `ENGINES_UP === panes.length` can then never be true, and the wait below ran its whole
 * 30 s ceiling twice - 6 s alone, 112 s in the lane - before the ⌘-digit half found the ring and
 * the caret on that leftover pane's URL field (#205).
 */
const engineCount = (panes) => panes.filter((pane) => pane.type === 'shell').length;

/**
 * Wait for at least `wanted` engines to be up, and say so when they never are.
 *
 * `>=`, not `===`, and the difference is the one sandbox the battery shares. `ENGINES_UP` counts
 * hidden terminal textareas in the whole document, and what draws a terminal is whatever view the
 * `terminal` workbench slot names: a scenario that has run before this one may have left a PLUGIN
 * renderer selected, and a plugin renderer builds its own textarea inside an iframe (so it is not
 * counted) or an extra one beside the bundled pane's (so it is counted twice). An exact count turns
 * either into a 30 s wait that can never end, and then into assertions read off a half-built grid -
 * the `defaults.length` version of this wait is what cost the ⌘-digit half 112 s and a red ring in
 * every full lane (#205).
 *
 * A ceiling that runs out is recorded rather than swallowed: the checks below still run, and the
 * note says the grid they read was not the one this asked for.
 */
const enginesUp = async (page, d, rec, wanted, where) => {
    if (await d.settleDom(page, `${ENGINES_UP} >= ${String(wanted)}`, { ceilingMs: 30_000 })) return true;
    rec.note(`${where}: waited 30 s for ${String(wanted)} engine(s) and ${String(await page.eval(ENGINES_UP))} came up; the checks below read the grid as it stands`);
    return false;
};
...
export default async function ({ page, cli, rec, d, sleep }) {
    ...
    // What Default will actually put engines up for, which is not the same as how many panes it
    // holds. See `engineCount`.
    const defaultEngines = engineCount(defaults);
    rec.note(`Default holds ${String(defaults.length)} pane(s), ${String(defaultEngines)} of them terminals: ${JSON.stringify(defaults.map((pane) => pane.type))}`);

    /*
     * Workspace Two is this scenario's own and it goes when this scenario does. It is the LAST
     * scenario in the lane today, but "nothing runs after me" is not a licence: three panes and a
     * sidebar row left behind change the Default pane count, the engine count and every later
     * workspace's ⌘-digit ordinal for whoever is appended next (#205).
     */
    let twoID = null;
    try {
        const created = JSON.parse(await cli.ok(['workspace', 'create', '--name', 'Two', '--json']));
        twoID = created.workspace_id ?? created.id;
        ...
        await enginesUp(page, d, rec, 3, 'workspace Two coming up');
        ...
        await enginesUp(page, d, rec, defaultEngines, 'Default on the way out');
        ...
        await enginesUp(page, d, rec, 3, 'workspace Two after the row click');
        ...
        await clickWorkspaceRow(page, defaultID);
        await enginesUp(page, d, rec, defaultEngines, 'Default before the keyboard switch');
        ...
        await enginesUp(page, d, rec, 3, 'workspace Two after the keyboard switch');
        await sleep(1_200);
        ...
        if (twoID !== null) await cli.run(['workspace', 'delete', twoID, '--force']);
        // Deleting the workspace the window is on leaves it wherever the daemon puts it next, so
        // the window is put back on the workspace this scenario found it on.
        const row = `[data-testid="workspace-row"][data-workspace-id="${defaultID}"]`;
        if (await d.settleDom(page, `document.querySelector(${JSON.stringify(row)})`, { ceilingMs: 8_000 })) await clickWorkspaceRow(page, defaultID);
    }
}
