/**
 * `nex open`, `nex md`, `nex diff` (cli.md §13) — routing, not policy.
 *
 * `nex open` decides between a web pane and a markdown pane **in the CLI**, reusing wire
 ...
 *   3. anything else is a usage error pointing at `nex md` / `nex web open`.
 *
 * `nex md` skips step 1 entirely — it is the escape hatch that forces a markdown pane on any
...
/** Fire-and-forget `open` — the markdown route shared by `nex md` and `nex open`. */
...
export async function handleOpen(args: string[]): Promise<void> {
    const first = args[0];
    if (first !== undefined && isHelpToken(first)) {
        printLine('Usage: nex open [--here] <path-or-url>');
    ...
    if (argument === undefined || argument.startsWith('-')) {
        errLine('Usage: nex open [--here] <path-or-url>');
        exit(1);
    }

    const webTarget = webTargetForOpenArg(argument, routingContext());
    if (webTarget !== null) {
        if (reuse) errLine('nex open: --here is ignored for URLs (web panes always open in a new pane)');
    ...
    if (webOpenExtensions.has(extension)) {
        if (reuse) errLine('nex open: --here is ignored for web files (web panes always open in a new pane)');
    ...
    errLine(`nex open: don't know how to open ${shown}`);
    ...
    errLine('       Use `nex md <file>` to force a markdown pane, or `nex web open <url>`.');
    exit(1);
}
