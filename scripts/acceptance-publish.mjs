...
export function publicationPayload(report, pr) {
    if (report?.schemaVersion !== 1 || report.scope !== 'commit' || !commit(report.start?.head) || !canonicalRunId(report.runId)) throw new Error('requires a retained exact-commit acceptance report with canonical run identity');
    ...
    const components = result.assessments.map((c, i) => ({ label: publicId('component', i), kind: ['audit', 'scenario', 'vitest', 'command', 'build', 'unsupported-smoke'].includes(c.kind) ? c.kind : 'unknown', verdict: verdict(c.verdict), attempts: c.attempts.map(a => ({ verdict: verdict(a.verdict), counts: publicCounts(a.counts), outstandingVisualCount: a.outstandingVisuals.length, visualRequirementCount: a.visualRequirements.length, visualEvidence: a.visualRequirements.map((v,j) => ({ id: publicId('visual',j), placement: ['onscreen','offscreen','hidden'].includes(v.placement) ? v.placement : 'unknown', screenshotCount: count(v.shots.length), screenshots: v.shots.map(s => ({ sha256: hash(s?.sha256), placement: ['onscreen','offscreen','hidden'].includes(s?.placement) ? s.placement : 'unknown', blank: s?.blank === true })) })), missingEvidenceCount: count(a.missingCategories) })), retryCount: Math.max(0, c.attempts.length - 1), firstAttemptVerdict: verdict(c.attempts[0]?.verdict) }));
    ...
    return { schemaVersion: 1, context: 'kelpi/acceptance', sha: report.start.head, reference: commit(report.reference), runId: `run-${digest(report.runId).slice(0, 24)}`, verdict: result.verdict, state: result.verdict === 'verified' ? 'success' : result.verdict === 'failed' ? 'failure' : 'pending', description: `Strict acceptance: ${result.verdict}`,
        sourceClean: report.start.dirty.length === 0 && report.end.dirty.length === 0 && report.start.head === report.end.head, components, incidents,
